...
@Component
export default class EditInterviewer extends Vue{
  form = {
    id: '',
    employeeId: '',
    name: '',
  ...
  async created() {
    try {
      const res = await axios.get('http://localhost:8087/api/interviewer/' + this.$route.params.id);
      this.patchData(res)
    } catch (e) {
      console.error(e);
    }
  ...
  patchData(data) {
    ...
    this.form.employeeId = data.data.employeeId
